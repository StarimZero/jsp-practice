...
public class GoodsDAO {
	...
	public ArrayList<GoodsVO> list(QueryVO query, String uid){
		...
		try {
			String sql = "select *, (select count(*) from favorite f where uid=? and f.gid=g.gid) ucnt, (select count(*) from favorite f where f.gid=g.gid) fcnt from goods g where title like ? order by regDate desc limit ?, ?";
